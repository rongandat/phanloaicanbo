...
class Front_Model_Huyen extends Zend_Db_Table_Abstract {

    /**
     * Model Huyen
     * @author Nguyen Manh Hung
     */
    protected $_name = TABLE_HUYEN;
    protected $_id = 'huyen_id';
    ...
    public function fetchData($filters = array(), $sortFeild = null, $limit = null, $page = 1) {
        ...
        if (count($filters) > 0) {
            foreach ($filters as $feild => $keyword) {
                if ($feild == 'keyword') {
                    $select->where('huyen_name like?', $keyword . '%')->orWhere('huyen_name like?', '%' . $keyword . '%')->orWhere('huyen_name like?', '%' . $keyword)->group('huyen_id');
                } else {
