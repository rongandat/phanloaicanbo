...
class Front_Model_TieuChiDanhGiaCB extends Zend_Db_Table_Abstract {

    /**
     * Model Hoc Ham
     * @author Nguyen Manh Hung
     */
    protected $_name = TABLE_TIEUCHIDANHGIACANBO;
    protected $_id = 'tcdgcb_id';
    ...
    public function fetchData($filters = array(), $sortFeild = null, $limit = null, $page = 1) {
        ...
        if (count($filters) > 0) {
            foreach ($filters as $feild => $keyword) {
                if ($feild == 'keyword') {
                    $select->where('tcdgcb_name like?', $keyword . '%')->orWhere('tcdgcb_name like?', '%' . $keyword . '%')->orWhere('tcdgcb_name like?', '%' . $keyword)->group('tcdgcb_id');
                } else {
