...
class Front_Model_Bangcap extends Zend_Db_Table_Abstract {

    /**
     * Model Bang Cap
     * @author Nguyen Manh Hung
     */
    protected $_name = TABLE_BANGCAP;
    protected $_id = 'bc_id';
    ...
    public function fetchData($filters = array(), $sortFeild = null, $limit = null, $page = 1) {
        ...
        if (count($filters) > 0) {
            foreach ($filters as $feild => $keyword) {
                if ($feild == 'keyword') {
                    $select->where('bc_name like?', $keyword . '%')->orWhere('bc_name like?', '%' . $keyword . '%')->orWhere('bc_name like?', '%' . $keyword)->group('bc_id');
                } else {
