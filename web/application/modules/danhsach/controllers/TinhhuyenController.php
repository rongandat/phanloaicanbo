...
class Danhsach_TinhhuyenController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        //kiem tra permission
        $check_permission = $this->_helper->global->checkPermission($identity->group_id, '5005');
        if (!$check_permission) {
    ...
    public function xuatAction() {
        ...
        $this->view->title = 'Lọc danh sách theo tỉnh - ' . $translate->_('TEXT_DEFAULT_TITLE');
        ...
        $filterModel = new Front_Model_Tinh();
        $list_filters = $filterModel->fetchData(array('tinh_status'=>1));
        $filters = array();
        if($filter_selected) $filters['em_dia_chi_tinh'] = $filter_selected;
        ...
        if (sizeof($list_items)) {
            ...
            header('Content-Disposition: attachment;filename="Bang_Thong_Tin_Nhan_Vien_Theo_Tinh.xls"');
