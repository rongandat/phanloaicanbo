...
class Front_Model_ChucVuDoan extends Zend_Db_Table_Abstract {

    /**
     * Model Chuc Vu Doan
     * @author Nguyen Manh Hung
     */
    protected $_name = TABLE_CHUCVUDOAN;
    protected $_id = 'cvdoan_id';
    ...
    public function fetchData($filters = array(), $sortFeild = null, $limit = null, $page = 1) {
        ...
        if (count($filters) > 0) {
            foreach ($filters as $feild => $keyword) {
                if ($feild === 'keyword') {
                    $select->where('cvdoan_name like?', $keyword . '%')->orWhere('cvdoan_name like?', '%' . $keyword . '%')->orWhere('cvdoan_name like?', '%' . $keyword)->group('cvdoan_id');
                } else {
